<template>
	<view class="page">
		...
		<view class="u-flex user-box u-p-30">
			...
			<view class="u-flex-1" >
				<view class="u-font-18 u-p-b-20">{{userinfo.name || '点击登录'}}</view>
		...
		<view class="u-m-t-20">
			<u-cell-group>
				<u-cell-item icon="setting" title="设置" :iconStyle="iconStyle" @click="remove"></u-cell-item>
			</u-cell-group>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			userin(){
				this.$R.get('getInfo',{},this.token.header).then(res => {
					this.userinfo.name=res.user.nickName
