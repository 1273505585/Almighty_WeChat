<template>
	<view class="page">
		...
		<u-subsection :list="list" :current="index"  @change="login" ></u-subsection></view>
		<view class="p-5">
	<!-- 		<view  v-if="index==0?true:false">
			...
			<view>
			<view class="d-f align-stretch mb-5">
				<u-input type="text" v-model="user.username" maxlength="11" placeholder="请输入账号" class="border-bottom p-3 flex-1" placeholder-class="text-light-muted" />
	
			</view>
			<view class="d-f align-stretch mb-5">
				<u-input type="password" v-model="user.password" maxlength="20" placeholder="请输入密码" class="border-bottom p-3 flex-1" placeholder-class="text-light-muted" />
			...
			<view class="py-2 w-100 row bg-color rounded mb-4" :disabled="disabled"  :class="disabled ? 'bg-disabled-color' : 'bg-color'" @click="submit" :loading="loading">
				<u-loading mode="flower" color="success" :show="loading"></u-loading>
				{{ loading ? '登陆中..' : '登陆' }}
			</view>
			<view class="text-light-muted w-100 row-ac" style="height: 30rpx;">
				<view class="check-box row iconfont icon-gouxuan mr-2  border text-white rounded-50 font-s-1" @click="clickCheck" :class="checkIndex == true ? 'bg-color' : ''"></view>
	...
	methods: {
		...
		submit() {
			...
			if(this.index==1){  //账号密码模式登录检验
			   this.loading=true
			   this.$R.post('login',{username:this.user.username,password:this.user.password,code:this.user.code,uuid:this.user.uuid}).then(res => {
			   if(res.code==200){  
				   Promise.all([
				        uni.setStorage({
				        key: 'token',
